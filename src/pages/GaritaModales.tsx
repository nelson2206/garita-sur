...
import { limpiarCodigo, normPlate, nowIso, semaforo, uid, validarPase } from '../lib/util';
...
export function ValidarModal({ codigo, foto, onClose, onNoAnunciado }: { codigo: string; foto: string; onClose: () => void; onNoAnunciado: (unidadId?: string) => void }) {
  ...
  async function registrar(e: FormEvent) {
    ...
    try {
      await registrarIngreso(p, perfil!, { nombre: pase!.nombre, placa: normPlate(placa), unidadId: pase!.unidad_id, medio: 'qr', autorizo: (pase!.tipo === 'personal' ? 'Credencial recurrente · ' : 'Pase de invitado · ') + (u?.propietario || '') + (doc ? ` · doc ****${doc}` : ''), pase: pase!, foto, tipo: pase!.tipo });
      toast('Ingreso registrado'); onClose();
    } catch (err) { toast('No se pudo registrar: ' + (err as Error).message); } finally { setOcupado(false); }
  }
  return <Modal onClose={onClose}>
    <span className="eyebrow">Pase válido · {pase.tipo}</span><h3>{pase.nombre}</h3>
    <p className="small">{u?.lote} · {u?.propietario} {pase.rol ? `· ${pase.rol}` : ''}</p>
    <SemaforoBox s={semaforo(u, c)} titulo="Estado de cuenta del lote" />
    {foto && <div className="row"><img className="thumb" src={foto} alt="Foto capturada" /><span className="small">Foto para la bitácora (se borra a los {c?.retencion_dias ?? 30} días).</span></div>}
    <form onSubmit={registrar} className="grid">
      ...
      <button className="btn primary block" disabled={ocupado}>Registrar ingreso</button>
    </form>
    <button className="btn ghost" onClick={onClose}>Cancelar</button>
  </Modal>;
}
...
export function NoAnunciadoModal({ unidadId, onClose }: { unidadId?: string; onClose: () => void }) {
  ...
  async function enviar(e: FormEvent) {
    ...
    toast('Solicitud enviada al residente'); onClose();
  }
  return <Modal onClose={onClose}>
    ...
    <p className="muted">Si el residente contesta por teléfono, marca "Aprobó por teléfono" en la lista de solicitudes.</p>
    <button className="btn ghost" onClick={onClose}>Cancelar</button>
  </Modal>;
}
